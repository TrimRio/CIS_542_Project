...
def main():
    """Main CLI interface."""
    if len(sys.argv) < 2:
        print("Usage: python fat32_recovery.py <disk_image> [command]")
        print("\nSupported formats:")
        ...
        print("\nExample:")
        print("  python fat32_recovery.py disk.img deleted")
        print("  python fat32_recovery.py evidence.e01 scan")
        print("  python fat32_recovery.py disk.img recover 1000 5242880 recovered.jpg")
        print("  python fat32_recovery.py disk.img extract 1048576 recovered.jpg")
        sys.exit(1)
    ...
    try:
        ...
        if command == "deleted":
            ...
            if deleted_files:
                ...
                print("  python fat32_recovery.py <image> recover <cluster> <size> <output_file>")
                for df in deleted_files[:10]:
                    output_name = df['name'].replace('?', '_')
                    print(f"  python fat32_recovery.py {image_path} recover {df['start_cluster']} {df['size']} {output_name}")
            ...
            print("  python fat32_recovery.py <image> extract <offset> <output_file>")

        elif command == "recover":
            if len(sys.argv) < 6:
                print("Usage: python fat32_recovery.py <image> recover <cluster> <size> <output_file>")
                sys.exit(1)
            ...
            if len(sys.argv) < 5:
                print("Usage: python fat32_recovery.py <image> extract <offset> <output_file>")
                sys.exit(1)
